...
When an assistant‐initiated outbound call is forwarded to another recipient, the correct caller ID (assistant’s phone number) is now preserved. The receiving user will no longer see the forwarded user’s caller ID.
