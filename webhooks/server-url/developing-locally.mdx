...
<Frame caption="Routing server URL payloads to a public reverse proxy, which tunnels to our local development server.">
  <img src="/images/server-url/reverse-proxy-developing-locally.png" />
</Frame>

## The Problem

When Interactly dispatches events to a server, it must be able to reach the server via the open Internet.
...
<Steps>
  ...
  <Step title="Trigger Call Events">
    We will conduct normal calls on Interactly to trigger events. These events will go to the Ngrok URL & get tunnelled to our local machine.
...
<Frame caption="Terminal after running the 'ngrok' command forwarding to localhost:8080 — the 'Forwarding' URL is what we want.">
  <img src="/images/server-url/ngrok-cli-ui.png" />
</Frame>
...
You can now use this as a server URL in the various places you can [set server URLs](/webhooks/server-url/setting-server-urls) in Interactly.
...
<Frame caption="Logging call events routed to our local environment.">
  <img src="/images/server-url/logging-events-locally.png" />
</Frame>

Feel free to follow any of our [quickstart](/quickstart/dashboard) guides to get started with building assistants & conducting calls.
...
- Ensure your path on top of the base forwarding url is set correctly (ex: `/callbacks/Interactly`)
